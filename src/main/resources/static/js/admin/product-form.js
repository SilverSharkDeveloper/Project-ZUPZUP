...
submitBtn.addEventListener("click", productForm)
...
function productForm(e) {
    ...
    let representingTitle = document.querySelector(".representing-title");

    if(!file.value){
        check= false;
        representingTitle.innerHTML = representingTitle.innerHTML.replace(" *","")+" *";
        representingTitle.style.color ="red";
        $(".search-notice-img").css("border","1px solid red");
    }else{
        representingTitle.innerHTML =representingTitle.innerHTML.replace(" *","");
        representingTitle.style.color ="black";
        $(".search-notice-img").css("border","none");
    }
